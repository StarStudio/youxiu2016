...
<?php
include 'conn.php';
  $sql="select id,title from roll ORDER BY ordernum";
...
$i=1;
  ...
  {
		echo " <li class='list-group-item'><span scope='row'>$i</span>&nbsp;&nbsp;&nbsp;<a   href='../roll.php?id=$row[id]'><h4 style='display:inline;' >$row[title]</h4></a></li><a style='padding-left: 400px;' type='button' href='rolledit.php?id=$row[id]'>编辑</a></br>"; 
		$i++;
  }
       
 ?>
  <a type='button' class='btn btn-default' href='order.php?table=roll' >顺序修改</a>
</ul>
  </br>
  </br>
<div class="panel panel-default col-xs-offset-2 col-xs-8">
<div class="panel-heading">发表</div>
<form class="form-horizontal col-xs-9" enctype="multipart/form-data" name="input" action="rollsub.php" method="post">
